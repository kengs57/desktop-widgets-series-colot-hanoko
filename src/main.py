...
class State(Enum):
    ...
    SINGING = auto()    # 🎤 음악 재생 중 (노래하는 코냥이: 곧켁롯.jfif)
    EXHAUSTED = auto()  # 😵 음악 멈춘 후 3초간 (힘들어하는 코냥이: 켁롯.jfif)
...
class TransparentWindow(QMainWindow):
    """
    상태 머신(Idle, Typing, Afk, Singing, Exhausted), 오디오 감지,
    캐릭터 완전 고정 하단 앵커 & 말풍선 오버레이(3초), 음악 종료 3초 유지 기능 통합 위젯
    """
    ...
    def __init__(self, base_dir: Path):
        ...
        # =========================================================================
        # 🖼️ [상태별 이미지 파일 경로 매핑]
        # assets 폴더 내 실제 이미지 파일명 1:1 매핑 정돈
        # =========================================================================
        self.image_files = {
            # 1) 🎤 음악 재생 중 (Singing): 마이크 들고 노래하는 코냥이
            State.SINGING: [
                self.assets_dir / "곧켁롯.png",
                self.assets_dir / "고양이메이드롯.png",
            ],

            # 2) 😵 음악 멈춘 후 3초간 (Exhausted): 마이크 잡고 힘들어하는 코냥이
            State.EXHAUSTED: [
                self.assets_dir / "켁롯.png",
                self.assets_dir / "고양이메이드롯.png",
            ],
        ...
        # 3. 윈도우 컨테이너 고정 크기 지정 (말풍선이 생성되어도 전체 창과 캐릭터 위치가 흔들리지 않음)
        ...
        # 캐릭터 이미지를 표시할 Label (컨테이너 하단 중앙에 절대 고정)
        self.image_label = QLabel(central_widget)
        self.image_label.setAlignment(Qt.AlignmentFlag.AlignCenter)

        # 말풍선 이미지를 표시할 Label (캐릭터 상단 머리 위 오버레이)
    ...
    def update_fixed_layout(self):
        ...
        말풍선이 나타나거나 사라져도 캐릭터 위치가 단 1픽셀도 흔들리지 않도록 물리적 완전 고정.
        """
    ...
    def change_state(self, new_state: State):
        ...
        print(f"[State Machine] 상태 변경 -> {new_state.name}")
    ...
    def check_state_update(self):
        ...
        # 위젯 자체 말풍선 효과음이 나오는 4초 동안은 외부 음악 재생으로 오인하지 않도록 예외 차단
        is_self_sound_playing = (now - self.last_self_sound_time) < 4
    ...
    def play_meow_sound(self):
        """코롯 위젯 말풍선 효과음 재생 (8개 신규 음성 중 무작위 1개 재생)"""
        ...
        # 존재하는 음성 파일 목록 확인
        available_sounds = [
            self.sounds_dir / name for name in sound_names if (self.sounds_dir / name).exists()
        ]

        # 신규 8개 음성 중 파일이 없을 경우 sounds 폴더 내 다른 wav 파일로 대체 (Fallback)
        if not available_sounds:
            available_sounds = list(self.sounds_dir.glob("*.wav"))

        if available_sounds and AUDIO_ENABLED:
            sound_path = random.choice(available_sounds)
            try:
                sound = pygame.mixer.Sound(str(sound_path))
                sound.play()
                print(f"[Sound] 코롯 랜덤 효과음 재생: {sound_path.name}")
            except Exception as e:
                print(f"[Error] 사운드 재생 에러: {e}")
        else:
            print("[Notice] 코롯 효과음 파일을 찾을 수 없거나 오디오가 비활성화되었습니다.")
    ...
    def contextMenuEvent(self, event):
        ...
        status_action = QAction(f"📌 현재 상태: {status_name}", self)
        status_action.setEnabled(False)
        menu.addAction(status_action)

        menu.addSeparator()

        quit_action = QAction("🚪 위젯 종료 (Quit)", self)
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Escape:
            print("[System] Esc 키 입력으로 위젯을 종료합니다.")
